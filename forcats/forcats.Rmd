...
Factors play a pivotal role when dealing with **categorical data** (variables that have a fixed and known set of possible values) in R. Key points about factors:
...
- They have constrained valid values (e.g., months of the year).
- They're prone to inconsistencies and don't have meaningful sorting.

**Solution** - Use Factors!

```r
# Turning string into a factor:
months_vector <- c("Dec", "Apr", "Jan", "Mar")
month_levels <- month.name  # R's built-in constant for month names
ordered_months <- factor(months_vector, levels = month_levels)
```

**Perks** of Using Factors:

- Restricts values to a predefined set; others become NA.
- Provides intuitive sorting.
- Ordering based on data appearance is a breeze with `fct_inorder()` (see later).
...
## Lumping Factor Levels in `gss_cat` Data
...
To simplify plots or tables, we often lump together the small groups of a factor. The `fct_lump_lowfreq()` function from the `forcats` package serves this purpose by grouping the smallest categories into "Other".
...
1. **forcats**:
   - Official Documentation: [forcats.tidyverse.org](https://forcats.tidyverse.org/)
...
2. **janitor**:
   - Package on CRAN: [janitor package on CRAN](https://cran.r-project.org/web/packages/janitor/index.html)
   - Data cleaning with janitor: [Article link or tutorial link](#) _(You can replace this placeholder with any specific resource you have in mind)_
...
# Ready to become an expert in `forcats`?
